...
class Network():
    def __init__(self, ut):
        ...
        if startFunctionAttack == True:
            self.ut.viewsPrint("showMsgEndAttack", "[{}] - End Attack Loop.".format(os.path.basename(__file__)))
    ...
    def startFunctionAttack(self):
        ...
        for info_player in collect_scan_player:
            ...
            if firewall > int(p.getHelperApplication()["SDK"]["level"]):
                ...
                self.ut.viewsPrint("showMsgDoesntPossibleAttack", "[{}] - Can't Attack '{}'. Your SDK ({}) vs Target Firewall ({}) fails :(".format(os.path.basename(__file__),ip,int(p.getHelperApplication()["SDK"]["level"]), firewall))
                time.sleep(1)
            else:
    ...
    def createMalwareKit(self):
        ...
        if int(malware["result"]) == 0:
            if int(malware["tasksCount"]) != 1:
                ...
                self.ut.viewsPrint("showMsgGenerateMWK", "[{}] - \033[33m Creating MalwareKit, you have ({}) MalwareKits \033[0m".format(os.path.basename(__file__), malware["mwReady"]))
