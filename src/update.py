            ...
            if (count_update+update) < 9:
                ...
                for list_update in self.Configuration["update"]:
                    ...
                    if money >= int(applications["price"]):
                        ...
                        self.ut.viewsPrint("showMsgUpdate", "[{}] - you have no money to upgrade {}".format(os.path.basename(__file__), list_update))
                        time.sleep(0.5)
            else:
                self.ut.viewsPrint("showMsgUpdatefull", "[{}] - full task, please wait.".format(os.path.basename(__file__)))
                time.sleep(0.5)
