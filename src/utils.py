...
class Utils:
    def __init__(self):
        ...
        self.login = "0"
        self.sentryClient = Client(dsn='https://184c11134c2146f58d3908d52baa2c0e:216f42513e67452abb74c5924e55abbc@sentry.io/1194987', release="release-1.5-patch.2", tags = {'hash': raven.fetch_git_sha(os.path.dirname(__file__))}, ignore_exceptions = [ ValueError, KeyError, KeyboardInterrupt, ReadTimeout, ConnectTimeout ])
        try:
      ...
      if condition in self.Configuration["show_info"] or "All" in self.Configuration["show_info"]:
            if "!{}".format(condition) in self.Configuration["show_info"]:
              ...
              if self.Configuration["debug"]:
                print(self.printConsole("\033[0;103m\033[1;30mBot Log: {} - {}\033[0m".format(strftime("%Y-%m-%d %H:%M:%S", gmtime()), Msg)))
              else:
                return self.OutputTable("Bot Log: {} - {}".format(strftime("%Y-%m-%d %H:%M:%S", gmtime()), self.printConsole(Msg)), 2)
